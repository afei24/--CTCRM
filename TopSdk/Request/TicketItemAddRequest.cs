...
namespace Top.Api.Request
{
    ...
    /// </summary>
    public class TicketItemAddRequest : BaseTopRequest<Top.Api.Response.TicketItemAddResponse>
    {
        /// <summary>
        ...
        /// 核销机具提供商
        /// </summary>
        public string CheckToolMerchant { get; set; }
        ...
        /// <summary>
        /// 门票对应产品标识<br>  门票商品只能使用达尔文体系下的产品。该参数可通过taobao.products.search接口获取，注意返回产品的cspu_feature属性，该属性表明是否为达尔文体系下产品。
        /// </summary>
        ...
        /// </summary>
        public string SellerCsPhone { get; set; }
        ...
        /// <summary>
        /// 门票商品产品规格信息，包括门票的库存价格等信息（产品规格可通过tmall.product.specs.get接口获取）。<br>请通过门票商品使用的产品所对应的产品规格信息进行设置，注意由于产品规格信息比较复杂，所以直接使用json作为传输，请按照约定传入正确的json格式）<br>  格式定义：<br>  <pre>  {      "SKU": {          "20890017-121840019_20394-121290067": { // 产品下的某个产品规格的属性属性值信息（pid-vid_pid-vid）              "effDates": {                  "1": {                      "type": "0",                      // 有效期类型，0-非指定日票，1-指定日票，2-年卡                      "effDate": { // 有效期，如下几块，type已经去掉                          "startDate": "2013-01-01",                          // 有效期时间段开始时间,null代表未设置                          "endDate": "2013-12-31",                          // 有效期时间段结束时间,null代表未设置                          "weeks": ["1", "4"],                          // 有效期周,1~7代表周一到周日，null代表未设置                          "startHour": "12",                          // 有效期开始时间小时,null代表未设置                          "startMinute": "59",                          // 有效期开始时间分钟,null代表未设置                          "endHour": "18" // 有效期结束时间小时,null代表未设置                          "endMinute": "30" // 有效期结束时间分钟,null代表未设置                          "effDays": "1",                          // xx后n天内有效的天数，购买后、出票后、开卡后n天内有效,null代表未设置                      },                      "timeLimit": { // 入园时间限制数据                          "limit": {                              "type": "0",                              // 入园时间限制类型,0-不限，1-提前n天的n点n分，2-提前n小时n分钟                              "aheadDays": "3",                              // 入园时间限制类型为提前n天的n点n分时才使用，入园时间提前n天的天数                              "aheadAtHour": "12",                              // 入园时间限制类型为提前n天的n点n分时才使用，入园时间在n点n分之前的小时                              "aheadAtMinute": "30",                              // 入园时间限制类型为提前n天的n点n分时才使用，入园时间在n点n分之前的分钟                              "aheadHours": "48",                              // 入园时间限制类型为提前n小时n分时才使用，入园时间提前n小时的小时                              "aheadMinutes": "30",                              // 入园时间限制类型为提前n小时n分时才使用，入园时间提前n分钟的分钟                          }                          "autoActivate": { // 有效期类型为年卡时才使用                              "type": "0",                              // 是否有最晚自动开卡时间,0-无，1-有最晚自动开卡                              "time": "30" // 天数,无最晚自动开发时间时为null                          }                      }                        "price": 10000,                      "inventory": 100,                      "outerId": "商家编码"                  },                  "222": {}              },              "save": true,              // 必须，代表是保存的有效数据              "returnRule": {                  "type": "0",                  // 0,1,2                  "value": "" // type 0:""，1:""，2:"卖家输入的文本"              },              "changeRule": {                  "type": "1",                  // 0,1,2                  "value": "" // type 0:""，1:""，2:"卖家输入的文本"              },              "oldGuide": "读取商品以前的预定须知文本",              //商品以前的预定须知，可选，              "feeDesc": "卖家输入的文本",              //预定须知-费用包含说明，必选，              "orderTimeDesc": "卖家输入的文本",              //预定须知-预定时间说明，必选，              "checkinValue": "1",              //入园方式-选项，必选，1,"报验证码换票入园";2, "扫二维码入园";3, "扫身份证入园";4, "报手机号换票入园";5, "使用实体票入园";6, "其他"              "checkinDesc": "卖家输入的文本",              //入园方式-补填信息。当checkinValue为6时必选，其他情况可选              "sellerNoteTip": "卖家输入的文本",              //预留信息，买家在下单时，卖家留言处的提醒，必选，              "otherGuide": "卖家输入的文本" //预定须知其他补充说明，可选          }      }  }  </pre><br>
        /// </summary>
        ...
        public override IDictionary<string, string> GetParameters()
        {
            ...
            parameters.Add("check_tool_merchant", this.CheckToolMerchant);
            ...
            parameters.Add("seller_cs_phone", this.SellerCsPhone);
        ...
        {
            RequestValidator.ValidateRequired("auction_status", this.AuctionStatus);
            RequestValidator.ValidateMaxLength("auction_status", this.AuctionStatus, 10);
            RequestValidator.ValidateRequired("check_tool_merchant", this.CheckToolMerchant);
            ...
            RequestValidator.ValidateRequired("image_1", this.Image1);
            ...
            RequestValidator.ValidateRequired("seller_cs_phone", this.SellerCsPhone);
            RequestValidator.ValidateRequired("skus", this.Skus);
            RequestValidator.ValidateRequired("sub_stock_at_buy", this.SubStockAtBuy);
