...
@item_close_count,@last_trade_time,@item_num,@trade_amount,@grade,@province,@city,@trade_count,@createDate,@groupNo,@buyerType)";
        ...
        public static bool UpdateBuyerInfo(CrmMember o, string sellerNick, string groupNo)
        {
            try
            {
                string query = @"update Buyer set avg_price = @avg_price,status = @status,close_trade_count = @close_trade_count,
        ...
        public static bool UpdateGrade(Buyers o)
        {
            try
            {
                string query = @"update Buyer set grade = @grade,updateDate=getdate() WHERE buyer_id = @buyer_id and SELLER_ID = @SELLER_ID";
        ...
        public static String GetBuyerNick(string buyerID, string sellerNick)
        {
            try
            {
                string query = @"select buyer_nick from Buyer where buyer_id = @buyer_id and SELLER_ID = @SELLER_ID";
        ...
        public static bool Update(Buyers o)
        {
            try
            {
        ...
        public static Boolean CheckBuyerIsExit(string buyerID, string sellerNick)
        {
            try
            {
                string query = @"SELECT 1 FROM Buyer WHERE buyer_id = @buyer_id and SELLER_ID = @SELLER_ID";
                SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@buyer_id",buyerID),
                new SqlParameter("@SELLER_ID",sellerNick)
        ...
        public static DataTable GetBuyerByBuyerID(Buyers entity)
        {
            try
            {
                string query = @"SELECT B.buyer_nick,case B.grade WHEN 0 THEN '潜在会员' when 1 THEN '普通客户' WHEN 2 THEN '高级会员' WHEN 3 THEN 'VIP会员'
                                ...
                                B.last_trade_time,B.cellPhone,B.Phone,B.address,B.qq,B.birthDay,B.memo,B.buyer_reallName,B.province,B.email,B.city,B.zipCode,B.buyer_alipay_no,G.group_name,dbo.GetRefundBuyer(@buyer_nick,@SELLER_ID) as hasRefund FROM Buyer B LEFT JOIN UserGroup G ON B.group_id = G.groupID WHERE B.buyer_id=@buyer_id and B.SELLER_ID = @SELLER_ID";
                SqlParameter[] param = new SqlParameter[]
            {
        ...
        public static DataTable GetBuyerNoPhone(string sellerNick)
        {
            try
            {
                string query = @"select buyer_id,buyer_nick from Buyer where SELLER_ID = @sellerNick and cellPhone is null ";
                SqlParameter[] param = new SqlParameter[]
            {
        ...
        /// <returns></returns>
        public static string GetBuyerCountJifenByNick(string nick, string sellerNick)
        {
            try
            {
                string query = @"select sum(jifenHis) as jifenHis from BuyerJifen where buyer_nick = @buyer_nick and sellerNick = @sellerNick";
                SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@buyer_nick",nick),
                new SqlParameter("@sellerNick",sellerNick)
            };
                DataTable dt = DataBase.ExecuteDt(query, param, CommandType.Text);
                return dt.Rows[0]["jifenHis"] == null ? "0" : dt.Rows[0]["jifenHis"].ToString();
            }
            catch (Exception ex)
            {
                ExceptionReporter.WriteLog(ex, ExceptionPostion.TBApply_Data);
        ...
        public static string GetBuyerNickByID(string buyerID)
        {
            try
            {
                string query = @"select buyer_nick from Buyer where buyer_id= @buyer_id";
                SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@buyer_id",buyerID)
            };
        ...
        /// </summary>
        /// <returns></returns>
        public static DataTable GetBuyerTop10Info(string sellerNick)
        {
            try
            {
                string query = @"select top 10 buyer_nick,trade_amount,trade_count,item_num,avg_price from Buyer 
                                where SELLER_ID = @SELLER_ID order by trade_amount desc ";
                SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@SELLER_ID",sellerNick)
            };
        ...
        public static DataTable GetBuyerInfoBySellerNick(string sellerNick)
        {
            try
            {
                string query = @"select buyer_id,grade,trade_amount from Buyer where SELLER_ID = @SELLER_ID ";
                SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@SELLER_ID",sellerNick)
            };
        ...
        public static string GetBuyerIdByBuyerNick(string buyerNick, string sellerNick)
        {
            try
            {
                string query = @"select buyer_id from Buyer where buyer_nick = @buyer_nick and SELLER_ID = @SELLER_ID";
                SqlParameter[] param = new SqlParameter[]
            {
        ...
        {
            var totalCount = 0;
            var query = " select count(1)  as totalCount from Buyer where SELLER_ID = @SELLER_ID";
            SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@SELLER_ID",sellerNick)
            };
            ...
            string area, string tradeAmount1, string tradeAmount2, string sellerNick2, string drpDay, string tradePinNv, string buyCount)
        {
            try
            {
                string query = @"select buyer_id,buyer_nick,ISNULL(buyer_reallName,'unknown') AS buyer_reallName,cellPhone,status,grade,province,
                                trade_amount,group_id,item_num, CONVERT(varchar(100),last_trade_time, 23) as last_trade_time,memo from Buyer 
        ...
        public static DataTable GetExportBuyers(string SELLER_ID)
        {
            try
            {
                string query = @"select buyer_nick,ISNULL(buyer_reallName,'unknown') AS buyer_reallName,cellPhone,case grade when 0 then '潜在会员'
                                ...
                                from Buyer where SELLER_ID = @SELLER_ID";
                SqlParameter[] param = new SqlParameter[]
            {
        ...
        public static DataTable GetExportBuyers(string SELLER_ID, string startDate, string endDate, string area, string tradeAmountFrom, string tradeAmountTo)
        {
            try
            {
                string query = @"select buyer_nick,ISNULL(buyer_reallName,'unknown') AS buyer_reallName,cellPhone,case grade when 0 then '潜在会员'
        ...
        /// <param name="sellerNick"></param>
        /// <returns></returns>
        public Boolean CheckTheSameHPNoIsExit(string sellerNick, string cellphone)
        {
            try
            {
                string query = @"SELECT 1 FROM Buyer WHERE SELLER_ID = @sellerNick AND cellPhone = @cellPhone";
                SqlParameter[] param = new SqlParameter[]
            {
                new SqlParameter("@sellerNick",sellerNick),
