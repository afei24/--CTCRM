 ...
 */
(function ($) {
    //class为.wBox_close为关闭 .wBox_save为确定
    $.fn.wBox = function (options) {
        ...
        var wBoxHtml = '<div id="wBox"><div class="wBox_popup"><table><tbody><tr><td class="wBox_tl"/><td class="wBox_b"/><td class="wBox_tr"/></tr><tr><td class="wBox_b"><div style="width:10px;">&nbsp;</div></td><td><div class="wBox_body">' + (_this.YQ.noTitle ? '' : '<table class="wBox_title"><tr><td class="wBox_dragTitle"><div class="wBox_itemTitle">' + _this.YQ.title + '</div></td><td width="20px" title="关闭"><div class="wBoxclose wBox_close"></div></td></tr></table> ') +
        ...
        this.showBox = function () {
            $("#wBox_overlay").empty();
            $("#wBox_overlay").remove();
            $("#wBox").find("iframe").prop("src", "");
            $("#wBox").empty();
            $("#wBox").remove();

            B = $("<div id='wBox_overlay' class='wBox_hide' style='hight:150px'></div>").hide().addClass('wBox_overlayBG').css('opacity', _this.YQ.opacity).appendTo('body').fadeIn(300);
            ...
            if (_this.YQ.requestType && $.inArray(_this.YQ.requestType, ['iframe', 'ajax', 'img']) != -1) {
                ...
                if (_this.YQ.requestType === "img") {
                    ...
                    if (_this.YQ.requestType === "ajax") {
                        ...
                        ifr = $("<iframe name='wBoxIframe' style='width:" + _this.YQ.iframeWH.width + "px;height:" + _this.YQ.iframeWH.height + "px;' scrolling='" + _this.YQ.scrolling + "' frameborder='0' src='" + _this.YQ.target + "'></iframe>");
        ...
        */
        function afterHandleClick() {
            setPosition();
            C.show().find('.wBox_close').click(_this.close);
            C.show().find('.wBoxclose').hover(function () {
                $(this).addClass("on");
            }, function () {
                $(this).removeClass("on");
            });
        ...
        */
        function setPosition() {
            ...
            var width = C.width(), height=C.height(), lt = calPosition(width, height);
            C.css({
                left: lt[0],
                top: lt[1]
            });
            if (_this.YQ.parent == null) {
                var $h = $(document).height();
                B.height($h).width($win.width());
            }
            else {
                //alert($(_this.YQ.parent).offset().top);
                var pt = $(_this.YQ.parent).offset().top;
                var pl = $(_this.YQ.parent).offset().left;
                var ph = $(_this.YQ.parent)[0].offsetHeight;
                var pw = $(_this.YQ.parent)[0].offsetWidth;
                B.css("left", pl + "px");
                B.css("top", pt + "px");
                B.height(ph);
                B.width(pw);
        ...
        */
        function calPosition(w, h) {
            pl = ($win.width() - w) / 2;
            if (h < 200) h = 200;
            pt = $win.scrollTop() + ($win.height() - h) / 2;
            return [pl < 0 ? 0 : pl, pt < 0 ? 0 : pt];
        }
        /*
        * 拖拽函数drag
        */
        ...
        */
        this.close = function () {
            ...
            if ($(this).hasClass('wBox_save')) {
                typeof _this.YQ.save === 'function' ? _this.YQ.save() : null
            }
            if (C) {
                B.empty();
                B.remove();
                C.stop().fadeOut(300, function () {
                    C.find("iframe").prop("src", "");
                    C.empty();
                    C.remove();
                })
            }
