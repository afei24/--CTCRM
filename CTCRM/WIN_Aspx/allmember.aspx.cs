    ...
    public partial class allmember : System.Web.UI.Page
    {
        ...
        private void searchBuyers()
        {
            ...
            try
            {
                ...
                {
                    Convert.ToInt32(tradeAmount1);
                }
                if (!string.IsNullOrEmpty(tradeAmount2))
                {
                    Convert.ToInt32(tradeAmount2);
                }
                if (!string.IsNullOrEmpty(buyCount))
                {
                    Convert.ToInt32(buyCount);
                }
        ...
        protected void grdBuyer_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                //((HyperLink)e.Row.FindControl("HyperLinkEdit")).Attributes.Add("onclick", "javascript:var iTop2 = (window.screen.availHeight - 20 - 350) / 2; var iLeft2 = (window.screen.availWidth - 10 - 600) / 2; var params = 'menubar:no;dialogHeight=350px;dialogWidth=600px;dialogLeft=' + iLeft2 + 'px;dialogTop=' + iTop2 + 'px;resizable=yes;scrollbars=0;resizeable=0;center=yes;location:no;status:no';var bLogged = Dialog( 'buyerInfoDetails.aspx?buyer_id= " + grdBuyer.DataKeys[e.Row.RowIndex].Value.ToString() + "', 'window',params);");
                ((HyperLink)e.Row.FindControl("HyperLinkEdit")).Attributes.Add("onclick", "javascript:$('.moren').show();$('.close').show();$('.detail_div').attr('src', 'buyerInfoDetails.aspx?buyer_id= " + grdBuyer.DataKeys[e.Row.RowIndex].Value.ToString() + "');$('.detail_div').show()");
                //会员资料修改
                ...
                //((HyperLink)e.Row.FindControl("HyerLinkEditMember")).Attributes.Add("onclick", "javascript:var iTop2 = (window.screen.availHeight - 20 - 390) / 2; var iLeft2 = (window.screen.availWidth - 10 - 600) / 2; var params = 'menubar:no;dialogHeight=390px;dialogWidth=600px;dialogLeft=' + iLeft2 + 'px;dialogTop=' + iTop2 + 'px;resizable=yes;scrollbars=0;resizeable=0;center=yes;location:no;status:no';var bLogged = Dialog( 'memberDetails.aspx?buyer_id= " + grdBuyer.DataKeys[e.Row.RowIndex].Value.ToString() + "', 'window',params);");
                ((HyperLink)e.Row.FindControl("HyerLinkEditMember")).Attributes.Add("onclick", "javascript:$('.moren').show();$('.close').show();$('.detail_div').attr('src', 'memberDetails.aspx?buyer_id= " + grdBuyer.DataKeys[e.Row.RowIndex].Value.ToString() + "');$('.detail_div').show()");
            }
        }
        ...
        protected void grdBuyer_RowCommand(object sender, GridViewCommandEventArgs e)
        {
